...
class UltimateSocialDeuxPlacement {
	...
	private function __construct() {
		...
		if (class_exists('Woocommerce')) {
			add_action( 'woocommerce_before_single_product_summary', array( $this, 'woocommerce_top' ) );
			add_action( 'woocommerce_after_single_product_summary', array( $this, 'woocommerce_bottom' ), 15 );
		}
	...
	public function buttons_floating() {
		...
		if ( !( ( $hide_frontpage && is_front_page() ) || ( $hide_posts && $post_type == 'post' && !is_home() ) || ( $hide_blogpage && is_home() ) || ( $hide_pages && $post_type == 'page' ) || ( $hide_mobile && wp_is_mobile() ) || ( $hide_search && is_search() ) || ( $hide_archive && is_archive() ) ) && !in_array($post_id, $exclude, false) && ( $networks ) ) {
